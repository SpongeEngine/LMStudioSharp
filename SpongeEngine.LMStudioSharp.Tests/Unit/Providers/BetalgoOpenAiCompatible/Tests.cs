...
namespace SpongeEngine.LMStudioSharp.Tests.Unit.Providers.Betalgo
{
   ...
   {
       private readonly BetalgoOpenAiCompatibleProvider _openAiCompatibleProvider;
       ...
       public Tests(ITestOutputHelper output) : base(output)
       {
           ...
           _openAiCompatibleProvider = new BetalgoOpenAiCompatibleProvider(openAiService, logger: Logger);
       }
       ...
       public async Task ListModelsAsync_ShouldReturnModels()
       {
           ...
           var response = await _openAiCompatibleProvider.ListModelsAsync();
       ...
       public async Task GetModelAsync_ShouldReturnModel()
       {
           ...
           var model = await _openAiCompatibleProvider.GetModelAsync(modelId);
       ...
       public async Task CompleteAsync_ShouldReturnCompletion()
       {
           ...
           var response = await _openAiCompatibleProvider.CompleteAsync(request);
       ...
       public async Task ChatCompleteAsync_ShouldReturnChatResponse()
       {
           ...
           var response = await _openAiCompatibleProvider.ChatCompleteAsync(request);
           Logger.LogInformation("Response received: {Response}", JsonConvert.SerializeObject(response));
       ...
       public async Task StreamCompletionAsync_ShouldStreamTokens()
       {
           ...
           await foreach (var token in _openAiCompatibleProvider.StreamCompletionAsync(request))
           {
       ...
       public async Task StreamChatAsync_ShouldStreamTokens()
       {
           ...
           var receivedTokens = new List<string>();
           await foreach (var token in _openAiCompatibleProvider.StreamChatAsync(request))
           {
               receivedTokens.Add(token);
           }
       ...
       public async Task CreateEmbeddingAsync_ShouldReturnEmbedding()
       {
           ...
           var response = await _openAiCompatibleProvider.CreateEmbeddingAsync(request);
           Logger.LogInformation("Response received: {Response}", JsonConvert.SerializeObject(response));
       ...
       public async Task IsAvailableAsync_ShouldReturnTrue()
       {
           ...
           var isAvailable = await _openAiCompatibleProvider.IsAvailableAsync();
